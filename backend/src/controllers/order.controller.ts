...
export default class OrderController {
    async addOrder(req: Request, res: Response): Promise<Response> {
        ...
        try {
            const { id, name, email, masterId, cityId, watchSize, date, time, status, address, lngLat } = AddOrderSchema.parse(req.body);
            ...
            const order = await Order.create({
                id, watchSize, date, time, masterId, cityId, clientId: client.getDataValue('id'), status, endTime, price, reviewToken, imagesLinks, address, lngLat
            }, {
                transaction: addOrderTransaction
            });
