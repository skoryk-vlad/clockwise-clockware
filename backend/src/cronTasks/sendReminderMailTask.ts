...
export const sendReminderMailTask = async () => {
    cron.schedule(`0 0 ${9 + +process.env.SERVER_TIME_DIFFERENCE}-${17 + +process.env.SERVER_TIME_DIFFERENCE} * * *`, async () => {
        try {
            const orders = await Order.findAll({
                ...
                where: {
                    ...
                    time: getHours(new Date()) + 1 - +process.env.SERVER_TIME_DIFFERENCE
                }
            })
